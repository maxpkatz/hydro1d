    ...
    do i = Uin_l%grid%lo, Uin_l%grid%hi+1
       ...
       if (ustar > ZERO) then
          ...
          if (pstar > p_l) then
             ...
             if (lambda_l < ZERO .and. lambdastar_l < ZERO) then
             ...
             else if (lambda_l > ZERO .and. lambdastar_l > ZERO) then
          ...
          if (pstar > p_r) then
             ...
             ! the wave is a rarefaction
             if (lambda_r < ZERO .and. lambdastar_r < ZERO) then
                ! rarefaction fan is moving to the left -- solution is
                ! R state
                rho_state = rho_r
                u_state = u_r
             ...
             else if (lambda_r > ZERO .and. lambdastar_r > ZERO) then
                ! rarefaction fan is moving to the right -- solution
                ! is *R state
                rho_state = rhostar_r
                u_state = ustar
       ...
       fluxes%data(i,iuener) = rhoe_state*(u_state - vf % data(i,1)) + &
            0.5_dp_t*rho_state*u_state**2*(u_state - vf % data(i,1)) + p_state*(u_state - vf % data(i,1))
